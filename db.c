// C program for the above approach
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <windows.h>
#include <time.h>

// Structure of the employee
struct cus {
	char name[50];
	int age;
	int ac;
};
struct cus e;

// size of the structure
long int size = sizeof(e);

// In the start coordinates
// will be 0, 0
COORD cord = { 0, 0 };

// function to set the
// coordinates
void gotoxy(int x, int y)
{
	cord.X = x;
	cord.Y = y;
	SetConsoleCursorPosition(
		GetStdHandle(STD_OUTPUT_HANDLE),
		cord);
}

FILE *fp, *ft;

// Function to add the records
void addrecord()
{
	system("cls");
	fseek(fp, 0, SEEK_END);
	 char name[50];

	char another = 'y';
    int num,found,age;

	while (another == 'y') {
		printf("\nEnter Name : ");
		scanf("%s", name);

		printf("\nEnter Age : ");
		scanf("%d", &age);

		 

		printf("\nEnter ac no : ");
        scanf("%d",&num);
         rewind(fp);
		 while (fread(&e, size, 1, fp) == 1)
		 {
		  if(e.ac ==num){
		printf("\n AC no is already Exists\n");
		system("pause");
		  break;
		  }
		  else
		  e.ac = num;
		  e.age=age;
		  strcpy(e.name,name);
		 }
		  fwrite(&e, size, 1, fp);
		  printf("\ndata successfully saved ");
		  break;
		  
		//printf("\nWant to add another record (Y/N) : ");
		//fflush(stdin);

		//scanf("%c", &another);
	}
}

// Function to delete the records
void deleterecord()
{
	system("cls");
	char cusname[50];
	char another = 'y';
	int num;
	 
	 

	while (another == 'y') {
		printf("\nEnter cus "
			"name to delete : ");
		scanf("%s", cusname);
        printf("\nEnter AC no: ");
		scanf("%d", &num);
		if(strcmp(e.name,cusname) == 0  && e.ac==num) 
		{
		ft = fopen("temp.txt", "wb");
		rewind(fp);

		while (fread(&e, size,
					1, fp)
			== 1) {
			if (strcmp(e.name,
					cusname)
				!= 0)
				fwrite(&e, size, 1, ft);
				printf("\n\ndeleted");
				Sleep(3);
				
		}

		fclose(fp);
		fclose(ft);
		remove("data.txt");
		rename("temp.txt", "data.txt");
		fp = fopen("data.txt", "rb+");
		}
		else
		{
			printf("\nData file not found\n");
		}
		printf("\nWant to delete another"
			" record (Y/N) :");
		fflush(stdin);
		another = getche();
	}
}

// Function to display the record
void displayrecord()
{
	system("cls");

	// sets pointer to start
	// of the file
	rewind(fp);

	printf("\n========================="
		"==========================="
		"======");
	printf("\nNAME\t\tAGE\t\t"
		"\tAC\n",
		e.name, e.age,
		e.ac);
	printf("==========================="
		"==========================="
		"====\n");

	while (fread(&e, size, 1, fp) == 1)
		printf("\n%s\t\t%d\t\t%10d",
			e.name, e.age,e.ac);

	printf("\n\n\n\t");
	system("pause");
}
void search(){
	int num,found;
	  

      rewind(fp);
	 
	printf("Enter AC num to search: ");
	  scanf("%d", &num);
	 system("cls");
	 while (fread(&e, size, 1, fp) == 1){
		  if(e.ac == num){
			  found=1;
			  printf("\n========================="
		"==========================="
		"======");
			  printf("\nNAME\t\tAGE\t\t""\tAC\n",
		e.name, e.age,
		e.ac);
		printf("\n========================="
		"==========================="
		"======");
			 printf("\n%s\t\t%d\t\t%10d\n",e.name, e.age,e.ac);
          //   printf("\n\n\n\t");
		
		  system("pause");
		   
			  
		  }
	  }
	  if(found == 0){
	  printf("\ndata not found\n");
	  system("pause");
	  }
	  
	
}
// Function to modify the record
void modifyrecord()
{
	system("cls");
	char cusname[50];
	char another = 'y';
    char r;

	while (another == 'y') {
		printf("\nEnter cus name"
			" to modify : ");
		scanf("%s", cusname);

		rewind(fp);

		// While File is open
		while (fread(&e, size, 1, fp) == 1) {
			// Compare the employee name
			// with ename
			if (strcmp(e.name, cusname) == 0) {
				printf("\nEnter new name:");
				scanf("%s", e.name);
				printf("\nEnter new age :");
				scanf("%d", &e.age);
				 
				printf("\nEnter new cus ac :");
				scanf("%d", &e.ac);

				fseek(fp, -size, SEEK_CUR);
				fwrite(&e, size, 1, fp);
				break;
			}
            else   {
                printf("Data not found: %s\n",cusname);
                printf("want to add(y/n):");
                printf("if record is add then just ignore it and continue\n");
                 fflush(stdin);
                scanf("%c",&r);
                if(r=='y')
                addrecord();
                else
                break;

            }
		}

		// Ask for modifying another record
		printf("\nWant to modify another"
			" record (Y/N) :");
		fflush(stdin);
		scanf("%c", &another);
	}
}

// Driver code
int main()
{
	int choice;
     system("cls");
	// opening the file
	fp = fopen("data.txt", "rb+");

	// showing error if file is
	// unable to open.
	system("cls");
	if (fp == NULL) {
		fp = fopen("data.txt", "wb+");
		if (fp == NULL) {
			printf("\nCannot open file...");
			exit(1);
		}
	}

	system("Color 3F");
	printf("\n\n\n\n\t\t\t\t============="
		"============================="
		"===========");
	printf("\n\t\t\t\t~~~~~~~~~~~~~~~~~~~"
		"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
		"~~~~~");
	printf("\n\t\t\t\t==================="
		"============================="
		"=====");
	printf("\n\t\t\t\t[|:::>:::>:::>::> "
		"cus RECORD <::<:::<:::"
		"<:::|]\t");
	printf("\n\t\t\t\t==================="
		"============================="
		"=====");
	printf("\n\t\t\t\t~~~~~~~~~~~~~~~~~~~~"
		"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
		"~~~");
	printf("\n\t\t\t\t====================="
		"==============================\n");
	printf("\n\n\n\t\t\t\t\t\t\t\t\t\t"
		"Developer : "
		"\n\n\t\t\t\t");

	system("pause");

	while (1) {
		// Clearing console and asking the
		// user for input
		system("cls");
		gotoxy(30, 10);
		printf("\n1. ADD RECORD\n");
		gotoxy(30, 12);
		printf("\n2. DELETE RECORD\n");
		gotoxy(30, 14);
		printf("\n3. DISPLAY RECORDS\n");
		gotoxy(30, 16);
		printf("\n4. MODIFY RECORD\n");
		gotoxy(30, 18);
		printf("\n5.SEARCH");
		gotoxy(30,20);
		printf("\n6. EXIT\n");
		gotoxy(30, 22);
		printf("\nENTER YOUR CHOICE...\n");
		fflush(stdin);
		scanf("%d", &choice);

		// Switch Case
		switch (choice) {
		case 1:

			// Add the records
			addrecord();
			break;

		case 2:

			// Delete the records
			deleterecord();
			break;

		case 3:

			// Display the records
			displayrecord();
			break;

		case 4:

			// Modify the records
			modifyrecord();
			break;
        case 5:
		     search();
			 break;

		case 6:
			fclose(fp);
			exit(0);
			break;

		default:
			printf("\nINVALID CHOICE.......\n");
		}
	}

	return 0;
}
